...
function ResetPass() {
  ...
  const handleSubmit = () => {
    ...
    axios.put("http://localhost:5000/api/resetPass", reqBody)
    .then(res => {
      if(res.data.status){
        navigate("/login")
      }
    ...
    <div className='flex justify-center items-center min-h-screen bg-custom-gradient1'>
      <div className='flex flex-col px-8 py-4 bg-transparent rounded-xl shadow-2xl w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl'>
        ...
        <div className='flex flex-col space-y-2 w-full'>
          ...
          <div className={`flex flex-row w-full bg-white rounded border border-gray-300 ${focus.first ? "ring-2 ring-violet-700" : ""}`}>
            <input
              type={showPass.first ? 'text' : 'password'}
              className='w-9/10 p-2  focus:outline-none'
              required
              onChange={e => {setPass({...pass, first:e.target.value})}}
        ...
        <div className='flex flex-col space-y-2 w-full mt-3'>
          ...
          <div className={`flex flex-row w-full bg-white rounded border border-gray-300 ${focus.second ? "ring-2 ring-violet-700" : ""}`}>
            <input
              type={showPass.second ? 'text' : 'password'}
              className='w-9/10 p-2  focus:outline-none'
              required
              onChange={e => {setPass({...pass, second:e.target.value})}}
        ...
        <div className='flex flex-col space-y-2 w-full mt-8'>
          <button onClick={handleSubmit} className='p-2 rounded bg-blue-600 text-lg hover:bg-blue-800 hover:text-white'>Submit</button>
        </div>
      </div>  
    </div>
  )
