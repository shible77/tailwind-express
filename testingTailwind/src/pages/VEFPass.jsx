...
function VEFPass() {
    ...
    const submitCode = () => {
        ...
        axios.post("http://localhost:5000/api/verifyCode", reqBody)
        .then(res => {
            if(res.data.status){
        ...
        <div className='flex flex-col px-8 py-4 rounded-xl shadow-2xl bg-transparent w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl'>
            ...
            <div className='flex flex-col w-full space-y-3'>
                ...
                <div className='flex flex-row w-full justify-center items-center'>
                    ...
                    />
                    <button onClick={submitCode} className='p-2 bg-green-600 hover:bg-green-900 w-1/5 ml-1 rounded hover:text-white'>Verify</button>
                </div>
            </div>
            <p className='mt-5'>If not get the code? <button className='text-blue-600 hover:text-blue-800'>Resend</button></p>
        </div>
    </div>
  )
}
